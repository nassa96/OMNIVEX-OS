/**
 * SAINT V48 — MULTI-GOVERNOR DEBATE SYSTEM
 * ----------------------------------------
 * Multiple decision brains vote before execution
 */

class MultiGovernorV48 {

  constructor(governors = {}) {

    this.governors = governors;

    this.weights = {
      CONSERVATIVE: 1.5,
      AGGRESSIVE: 1.0,
      ADAPTIVE: 1.3,
      CONTRARIAN: 0.8,
      RISK_OFF: 1.7
    };
  }

  // =====================================================
  // RUN ALL GOVERNORS
  // =====================================================
  evaluate(state) {

    const votes = [];

    for (const name in this.governors) {

      const gov = this.governors[name];

      const decision = gov.decide(state);

      votes.push({
        name,
        decision,
        weight: this.weights[name] || 1.0
      });
    }

    return votes;
  }

  // =====================================================
  // NORMALIZE VOTES INTO ACTION SCORE
  // =====================================================
  resolve(votes) {

    let score = {
      HOLD: 0,
      REDUCE_EXPOSURE: 0,
      SELECTIVE_EXECUTION: 0,
      FULL_EXECUTION: 0
    };

    for (const v of votes) {

      score[v.decision] += v.weight;
    }

    return score;
  }

  // =====================================================
  // FINAL SYSTEM DECISION
  // =====================================================
  decide(state) {

    const votes = this.evaluate(state);

    const score = this.resolve(votes);

    const finalAction = Object.entries(score)
      .sort((a, b) => b[1] - a[1])[0][0];

    return {
      votes,
      score,
      finalAction
    };
  }

  // =====================================================
  // SNAPSHOT
  // =====================================================
  snapshot() {

    return {
      governorCount: Object.keys(this.governors).length,
      weights: this.weights
    };
  }
}

module.exports = MultiGovernorV48;
